...
class Swap(Wallet):
    ...
    def __init__(self, rpc_url: str, private_key: str) -> None:
        ...
        self.swapClient = Jupiter(
            async_client=self.client, keypair=self.wallet)

    async def get_token_mint_account(self, token_mint: str) -> Pubkey:
        print("get_token_mint_account")
        token_mint_account = get_associated_token_address(
            owner=self.wallet.pubkey(), mint=Pubkey.from_string(token_mint))
    ...
    async def swap_status(self, transaction_hash):
        ...
        try:
            # TBD
            return await self.get_status_transaction(transaction_hash=transaction_hash)
